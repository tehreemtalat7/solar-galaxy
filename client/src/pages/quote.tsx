...
export default function Quote() {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (!formData.firstName || !formData.lastName || !formData.email || !formData.phone || !formData.propertyType || !formData.monthlyBill) {
      toast({
        title: "Missing Information",
    ...
    const subject = encodeURIComponent(`Solar Quote Request from ${formData.firstName} ${formData.lastName}`);
    const body = encodeURIComponent(
      ...
      `Monthly Bill Range: PKR ${formData.monthlyBill}\n` +
      `Home Size: ${formData.homeSize || 'Not provided'} sq ft\n` +
      `Roof Type: ${formData.roofType || 'Not specified'}\n\n` +
      (quoteResults ? 
        `CALCULATED QUOTE:\n` +
        `Recommended System Size: ${quoteResults.systemSize} kW\n` +
        `Estimated Cost: PKR ${quoteResults.estimatedCost.toLocaleString()}\n` +
        `Monthly Savings: PKR ${quoteResults.monthlySavings.toLocaleString()}\n` +
        `Payback Period: ${quoteResults.paybackPeriod} years\n`
        : '')
    );
    ...
    toast({
      title: "Opening Gmail",
      description: "Please send the email to submit your quote request.",
    });
  };
                        ...
                        <div className="grid grid-cols-2 gap-3 sm:gap-4">
                          ...
                          <div>
                            ...
                            <div className="text-lg sm:text-xl md:text-2xl font-bold text-solar-orange">
                              PKR {quoteResults.estimatedCost.toLocaleString()}
                            </div>
                          </div>
                          <div>
                            <div className="text-xs sm:text-sm text-gray-600">Monthly Savings</div>
                            <div className="text-lg sm:text-xl md:text-2xl font-bold text-solar-green">
                              PKR {quoteResults.monthlySavings.toLocaleString()}
                            </div>
                          </div>
                          <div>
